# -*- coding: utf-8 -*-
"""
Created on Mon Jan 27 13:40:47 2020
...
if choice == '1':
    ...
    string = binary + checksum
    len(string)
    
    
    # Cut the binary number into 11-bit parts
    
    list = [''] * 12
    
    for bit in range(int(len(string)/11)):
        list[bit] = string[bit*11:bit*11+11]
    
    
    seed_words = [''] * 12
    
    # Convert from binary to decimal
    # For each element, we set a word from the BIP39 list.
    ...
    def importSeedMnemonique():
        ...
        for i in range(12):
            ...
            if valid == True:
                ...
                for j in range(2048):
                    if n == BIP39_list[j]:
                        tabDeMots.append(get_bin(j, 11))
                        break
                    if j == 2047 and tabDeMots[i] == '':
